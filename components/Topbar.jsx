  ...
  return (
    <header className="shell-topbar">
      <div className="shell-topbar-brand">
        <button type="button" className="shell-icon-btn shell-menu-btn" onClick={onMenuToggle} aria-label="Toggle menu">
        ...
        <Image src={logo1} alt="Engage Now Africa" className="shell-topbar-logo" priority />
        <div className="shell-topbar-title">
          <strong>Engage Now Africa</strong>
          <span>Project Management</span>
        </div>
      </div>

      <nav className="shell-topbar-nav" aria-label="Main navigation">
        {visibleCategories.map((cat) => (
          <button
            key={cat.id}
            type="button"
            className={`shell-nav-tab${activeCategory === cat.id ? ' active' : ''}`}
      ...
      <div className="shell-topbar-actions">
        <button type="button" className="shell-icon-btn" onClick={toggleTheme} aria-label="Toggle theme">
          {theme === 'dark' ? (
            <svg fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
