...
import { formatDate, timeAgo } from '@/lib/api-utils';
import { getReportStatusMeta, getReportTypeMeta } from '@/lib/report-types';
import { isProjectManager, LEADER_ROLE_DB_VALUES } from '@/lib/roles';
...
export function formatReport(r) {
  ...
  return {
    ...
    fileType: r.fileType,
    fileSize: r.fileSize,
    activityTable: parseReportTable(r.activityTable),
    driveLink: r.driveLink,
    editedAfterApproval: Boolean(r.editedAfterApproval),
    editedAfterApprovalAt: r.editedAfterApprovalAt,
    editedAfterApprovalLabel: r.editedAfterApproval
      ? `Edited after approved${r.editedAfterApprovalAt ? ` · ${formatDate(r.editedAfterApprovalAt)}` : ''}`
      : null,
    submittedAtLabel: r.submittedAt ? formatDate(r.submittedAt) : null,
    updatedAtLabel: r.updatedAt ? formatDate(r.updatedAt) : null,
    updatedAgo: r.updatedAt ? timeAgo(r.updatedAt) : null,
...
export function canEditReport(user, report) {
  ...
  return ['draft', 'revision_requested', 'rejected', 'approved', 'submitted', 'pending_approval'].includes(report.status);
}
